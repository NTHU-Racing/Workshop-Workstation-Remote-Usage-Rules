...
這是一個關於如何管理與遠端連接工廠電腦的説明頁面。為了讓大家能順利使用資源，請務必詳讀以下規則。

## !!Rules to know!! ⚠️

工廠目前有兩臺由 **生麗國際** 贊助的電腦供大家使用。由於辦公位子有限，我們開放讓大家可以 Remote Access（遠端存取）這些電腦。

在使用工廠電腦前，請嚴格遵守以下事項：

1.  **Public Computer Protocol**
    顧名思義這是工廠電腦，是公共的電腦。你可以用作私人用途，但請確保使用完後把你不要的資料都刪除。需要的文件請轉移到 Cloud Drive，或是這兩臺電腦都配有 **HDD**，可以作爲長期存儲文件的位置。
...
3.  **Session Management**
    ...
    > **重要：** 如果有需要放著讓程式跑 (Run Program) 或者跑模擬 (Simulation) 的情況，請務必在對應的 **Team Affair** 群組中通知，以免有人關掉你辛苦的結晶。
...
* **步驟一：** 開啟 Remote Desktop Connection，輸入電腦 IP 以及 Username（參考下圖）。
* **步驟二：** 點擊左下角 **Show Options**（參考下圖）。
* **步驟三：** 點擊 **Allow me to save credentials**（參考下圖）。
* **步驟四：** 跳出輸入密碼畫面時，兩臺電腦的密碼皆為 `46804804`。可以勾選 **Remember me** 就不用重複輸入，然後點擊 **OK**。
* **步驟五：** 會跳出 Warning 問你是否真的要遠端遙控此電腦，勾選左下角的 **"Don't ask me again for connections to this computer"** 後按下 **Yes/OK** 即可連線。
...
*(請在此處放入截圖，例如：`![Step 1 Screenshot](./images/step1.png)`) *
